...
/*------------------------------------------------------------------------------
 ...
 \return int - Возвращает 0 в случае успеха
 ------------------------------------------------------------------------------*/
int EEPROM_write(unsigned int addr, unsigned int buf, unsigned int len)
{
  ...
  if (addr & 1)
  {
    ...
    sdata = (sdata & 0x00FF) | ((*(unsigned char *)buf) << 8); // Совмещаем с данными не принадлежащими записываемой области
  ...
  while (len > 0)
  {
    ...
    if (len == 1)
    {
      ...
      sdata = (sdata & 0xFF00) | (*(unsigned char *)buf); // Совмещаем с данными не принадлежащими записываемой области
      res = EEPROM_program_sector(addr, sdata);
      addr++;
      buf++;
      len--;
